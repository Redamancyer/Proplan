...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass Electron development arguments to Proplan and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'Proplan-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: Proplan [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`Proplan: ${MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'Proplan-user-data')

...
class DataCenter extends TypedEmitter<DataCenterEvents> {
  ...
  init(): void {
    const managedImageFolderPath = path.join(this.userDataPath, 'proplan-assets')
    const defaultData = {
      imageFolderPath: managedImageFolderPath,
    ...
    this.store.set('imageFolderPath', managedImageFolderPath)
  ...
  async setItem(key: string, value: unknown): Promise<void> {
    const { encryptKeys, serviceName } = this
    if (key === 'imageFolderPath') {
      value = path.join(this.userDataPath, 'proplan-assets')
    }

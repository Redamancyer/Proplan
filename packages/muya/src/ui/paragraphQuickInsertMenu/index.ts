...
export class ParagraphQuickInsertMenu extends BaseScrollFloat {
    ...
    private _search(text: string) {
        ...
        if (!muya.options.blockImageQuickInsert) {
            const basicBlocks = menuConfig.find(menu => menu.name === 'basic blocks');
            if (basicBlocks)
                basicBlocks.children = basicBlocks.children.filter(item => item.label !== 'block-image');
        }
    ...
    override selectItem({ label }: IQuickInsertMenuItem['children'][number]) {
        ...
        const triggerBlock = block
            ?? (activeBlock instanceof ParagraphContent ? activeBlock : null);
        if (!triggerBlock?.parent) {
            this.hide();
            return;
        }
        if (label === 'block-image') {
            replaceBlockByLabel({
                label: 'paragraph',
                block: triggerBlock.parent,
                muya,
            });
            muya.format('image');

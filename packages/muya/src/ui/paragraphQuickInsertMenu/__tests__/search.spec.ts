...
describe('paragraphQuickInsertMenu block image command', () => {
    it('clears the slash paragraph and invokes the existing image formatter', () => {
        const { muya, menu } = bootMenu();
        const imageItem = MENU_CONFIG
            .find(section => section.name === 'basic blocks')!
            .children
            .find(item => item.label === 'block-image')!;
        const format = vi.spyOn(muya, 'format');

        const block = menu._block!;
        block.text = '/image';
        block.setCursor(block.text.length, block.text.length, true);
        (menu as unknown as ParagraphQuickInsertMenu).selectItem(imageItem);
        muya.flush();

        expect(format).toHaveBeenCalledWith('image');

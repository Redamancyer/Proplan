...
type Options = {
    ...
    imagePathPicker?: (muya: Muya) => Promise<string>;
    /**
    ...
    imageAction?: (state: IState, muya: Muya) => Promise<string>;
    /** Whether remote image URLs should also pass through imageAction. */
    processRemoteImage?: (muya: Muya) => boolean;
} & IBaseOptions;
...
export class ImageEditTool extends BaseFloat {
    ...
    private _replaceImageAsync = async ({ alt, src, title }: IState) => {
        ...
        const shouldProcessRemote
            = !URL_REG.test(src) || this.options.processRemoteImage?.(this.muya) === true;
        if (!this.options.imageAction || !shouldProcessRemote) {
    ...
    private async _replaceImageWithUpload(alt: string, src: string, title: string) {
        ...
        const uploadedSrc = await this.options.imageAction!({ src, title, alt }, this.muya);
    ...
    private async _handleSelectButtonClick() {
        ...
        const path = await this.options.imagePathPicker(this.muya);
